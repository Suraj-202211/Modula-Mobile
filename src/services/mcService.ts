...
export const fetchMCVersions = async (): Promise<MCVersionManifest> => {
  try {
    const response = await fetch('/api/mc/versions');
    if (response.ok) {
      return await response.json();
    }
  } catch (err) {
    console.warn('Failed to fetch from local bridge API, trying Mojang direct', err);
  }

  // Try direct Mojang URL as second tier
  try {
    ...
    console.warn('Failed to fetch from Mojang metadata API, returning fallback versions', err);
  }

  // Final tier: offline fallback mock data
  return {
    latest: {
      release: '1.20.4',
      snapshot: '24w14a'
